...
class ScrapeOpsFakeBrowserHeaderAgentMiddleware:
    ...
    def process_request(self, request, spider):
        ...
        # Use .get() so it won’t throw KeyError if the header is missing
        ...
        request.headers['sec-fetch-mode'] = random_browser_header.get('sec-fetch-mode', '')  # <-- fix typo: "mod" -> "mode"
...
class MyProxyMiddleware:
    ...
    def process_request(self, request, spider):
        # Build proxy host
        proxy_host = f"http://{self.endpoint}:{self.port}"
